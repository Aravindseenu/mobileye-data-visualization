#This code recives can data and calculates lane width and prints on the terminal
# this code has to executed on seperate terminal "python mobileye_width.py"
# this works properly on python2
import subprocess
import math 
...
class MobileyeData(): 
    ...
    def get_lanes(self):
        ...
        # the lane information is recived  
        llist1 = [self.current_lx[i] for i in range(len(self.current_lx))]
        rlist1 = [self.current_rx[i] for i in range(len(self.current_rx))]

        #lane width calculation by adding the abs values of obtained points
        for x in range(0, len(rlist1)):
            print("Current lane width") 
            print(abs(llist1[x])+abs(rlist1[x]))
             
        
        return {"left": llist1, "right": rlist1}
...
if __name__ == '__main__':
    ...
    for read_can in mobileye_parser.parse_data():
        if read_can is None: 
            print("verify the code")
        else: 
            print("data recived") 
